...
- 4択単一正答とする
- 正答は必ず1つだけにする
- 問題文だけで解答に必要な条件は提示するが、本文中に正答を露骨に示しすぎない
...
【誤答の作り方】
- 誤答は、否定を用いて外すのではなく、1か所を類似概念に置き換えて作る

【誤答の整合性ルール】
- 問題文に明示された情報だけで即座に除外できる誤答は禁止する
- 「本文に書いてあることと矛盾しているから消せる」タイプの誤答は禁止する
...
【専門用語の表記ルール】
- 問題文と選択肢は基本的に日本語で記述する
- ただし、専門用語について日本語の一般的名称に確信が持てない場合は、無理に和訳せず英語のまま用いる
- 不自然な直訳は避ける
- 日本語で広く定着した用語は日本語を優先する
- 疾患名、検査名、病理所見、画像所見、電気生理用語、治療薬・治療手技の名称は、臨床現場・論文・ガイドラインで一般的な表記を優先する
...
6. 誤答が問題文との矛盾だけで除外できない
7. 専門用語の和訳が不自然な場合、英語表記に逃がせている
`;
...
- 全ての選択肢は問題文と表面的に整合していること
- 問題文に明示された情報だけで即除外できる誤答を作らないこと
- explanation では、正答の根拠に加えて、他の3選択肢がなぜ誤りかも簡潔に説明すること
- learning point が英語でも、専門用語は無理に不自然な日本語へ訳さないこと
- 専門用語の日本語表記が一般的か不確かな場合は、単語レベルでは英語表記とすること
`;
